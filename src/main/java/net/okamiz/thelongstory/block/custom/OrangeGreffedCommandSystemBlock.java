...
public class OrangeGreffedCommandSystemBlock extends Block{
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit){
        ...
        if(timer <= 0){
            if(!world.isClient()){
                player.addStatusEffect(new StatusEffectInstance(StatusEffects.FIRE_RESISTANCE, 3600, 1));
                timer = timerdelay;
            }
    ...
    public void appendTooltip(ItemStack stack, @Nullable BlockView world, List<Text> tooltip, TooltipContext options) {
        tooltip.add(Text.translatable("tooltip.thelongstory.orange_greffed_command_system"));
